#!/usr/bin/env python3
"""
BTC AI EA — V9.2 + TURTLE + BOOK HIGH-CAGR INTEGRATED V2
======================================================

INTEGRATED V2 — V9.2 baseline plus causal Turtle/Book overlays
---------------
...
# ============================================================================
# INTEGRATED V2: Original-Turtle adaptation + BOOK hierarchy + V9.2 blend
# ============================================================================

def _ohlc_resample(df1h: pd.DataFrame, rule: str, label: str = "right") -> pd.DataFrame:
    return df1h[["open","high","low","close","volume"]].resample(
        rule, closed="left", label=label
    ).agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna()


def _persistent_break_regime(df: pd.DataFrame, lookback: int) -> pd.Series:
    """Causal close-confirmed prior-high / prior-low regime, book-style."""
    hi = df.high.shift(1).rolling(lookback, min_periods=lookback).max()
    lo = df.low.shift(1).rolling(lookback, min_periods=lookback).min()
    state = 0
    vals = []
    for c, h, l in zip(df.close, hi, lo):
        if np.isfinite(h) and c > h:
            state = 1
        elif np.isfinite(l) and c < l:
            state = -1
        vals.append(state)
    return pd.Series(vals, index=df.index, dtype=float)


def build_book_hierarchy_4h(df1h: pd.DataFrame) -> pd.DataFrame:
    """
    BOOK-QUANT implementation:
      - Daily/Weekly/Monthly completed-bar prior-high/low breaks define major side.
      - Daily is primary; Weekly/Monthly add confidence.
      - Signals are merged backward to 4H opens and then shifted one 4H bar before use.
    """
    d = _ohlc_resample(df1h, "1D", "right")
    w = _ohlc_resample(df1h, "1W", "right")
    m = _ohlc_resample(df1h, "MS", "right")
    d["d_side"] = _persistent_break_regime(d, 20)
    w["w_side"] = _persistent_break_regime(w, 10)
    m["m_side"] = _persistent_break_regime(m, 6)

    h4 = _ohlc_resample(df1h, "4h", "left")
    out = h4[["open","high","low","close"]].copy()
    for fr, col in [(d,"d_side"),(w,"w_side"),(m,"m_side")]:
        left = out.reset_index().rename(columns={out.index.name or "index":"time"})
        right = fr[[col]].reset_index().rename(columns={fr.index.name or "index":"time"})
        out = pd.merge_asof(left.sort_values("time"), right.sort_values("time"),
                            on="time", direction="backward").set_index("time")
    out[["d_side","w_side","m_side"]] = out[["d_side","w_side","m_side"]].fillna(0.0)
    agree = (out.w_side == out.d_side).astype(float) + (out.m_side == out.d_side).astype(float)
    out["book_conf"] = np.where(out.d_side == 0, 0.0, (1.0 + agree) / 3.0)
    # One full 4H execution delay: no completed-bar information is used on the same 4H open.
    out["book_side_live"] = out.d_side.shift(1).fillna(0.0)
    out["book_conf_live"] = out.book_conf.shift(1).fillna(0.0)
    return out


def build_original_turtle_4h(df1h: pd.DataFrame,
                             risk_per_unit: float = 0.0075,
                             max_units: int = 4,
                             max_abs_exposure: float = 1.50) -> pd.DataFrame:
    """
    BTC adaptation of the classic Turtle rule set, evaluated on COMPLETED DAILY bars:
      System 1: 20-day entry / 10-day exit with skip-after-profitable-S1 rule.
      System 2: 55-day entry / 20-day exit.
      N = Wilder ATR(20).
      Initial stop = 2N.
      Pyramid: add 1 unit after each favorable +0.5N close move, max 4 units.

    To avoid intraday sequencing assumptions, breakout, stop, and add decisions are
    confirmed at the daily close and become tradable only from the NEXT 4H open.
    That is deliberately more conservative than assuming perfect intraday stop fills.
    """
    d = _ohlc_resample(df1h, "1D", "right")
    d["N"] = atr(d, 20)
    d["hi20"] = d.high.shift(1).rolling(20, min_periods=20).max()
    d["lo20"] = d.low.shift(1).rolling(20, min_periods=20).min()
    d["hi55"] = d.high.shift(1).rolling(55, min_periods=55).max()
    d["lo55"] = d.low.shift(1).rolling(55, min_periods=55).min()
    d["exit10_hi"] = d.high.shift(1).rolling(10, min_periods=10).max()
    d["exit10_lo"] = d.low.shift(1).rolling(10, min_periods=10).min()
    d["exit20_hi"] = d.high.shift(1).rolling(20, min_periods=20).max()
    d["exit20_lo"] = d.low.shift(1).rolling(20, min_periods=20).min()
    ...
    last_add = np.nan
    stop = np.nan
    last_s1_profitable = False
    trade_entry = np.nan
    rows = []

    for ts, r in d.iterrows():
        c = float(r.close)
        n = float(r.N) if np.isfinite(r.N) else np.nan

        # Exit / stop from completed daily bar.
        exited = False
        if side > 0 and np.isfinite(n):
            channel_exit = (system == 1 and np.isfinite(r.exit10_lo) and c < r.exit10_lo) or \
                           (system == 2 and np.isfinite(r.exit20_lo) and c < r.exit20_lo)
            stop_exit = np.isfinite(stop) and c <= stop
            if channel_exit or stop_exit:
                pnl = c - trade_entry if np.isfinite(trade_entry) else 0.0
                if system == 1:
                    last_s1_profitable = pnl > 0
                side = system = units = 0
                entry = last_add = stop = trade_entry = np.nan
                exited = True
        elif side < 0 and np.isfinite(n):
            channel_exit = (system == 1 and np.isfinite(r.exit10_hi) and c > r.exit10_hi) or \
                           (system == 2 and np.isfinite(r.exit20_hi) and c > r.exit20_hi)
            stop_exit = np.isfinite(stop) and c >= stop
            if channel_exit or stop_exit:
                pnl = trade_entry - c if np.isfinite(trade_entry) else 0.0
                if system == 1:
                    last_s1_profitable = pnl > 0
                side = system = units = 0
                entry = last_add = stop = trade_entry = np.nan
        ...
        # Entry. System 1 is skipped after a profitable S1 trade; System 2 remains available.
        if side == 0 and not exited and np.isfinite(n):
            s1_long = np.isfinite(r.hi20) and c > r.hi20 and not last_s1_profitable
            s1_short = np.isfinite(r.lo20) and c < r.lo20 and not last_s1_profitable
            s2_long = np.isfinite(r.hi55) and c > r.hi55
            s2_short = np.isfinite(r.lo55) and c < r.lo55
            if s1_long or s2_long:
                side = 1; system = 1 if s1_long else 2; units = 1
                entry = last_add = trade_entry = c; stop = c - 2.0*n
            elif s1_short or s2_short:
                side = -1; system = 1 if s1_short else 2; units = 1
                entry = last_add = trade_entry = c; stop = c + 2.0*n

        # Winner-only pyramiding at 0.5N steps, confirmed by daily close.
        if side != 0 and units > 0 and units < max_units and np.isfinite(n) and np.isfinite(last_add):
            while units < max_units:
                trigger = last_add + side * 0.5*n
                if (side > 0 and c >= trigger) or (side < 0 and c <= trigger):
                    units += 1
                    last_add = trigger
                    # Classic Turtle convention: protective stop follows the newest unit by 2N.
                    stop = last_add - side * 2.0*n
                else:
                    break

        if side != 0 and np.isfinite(n) and c > 0:
            unit_exp = risk_per_unit / max(n/c, 1e-6)
            unit_exp = min(unit_exp, max_abs_exposure / max(max_units,1))
            exposure = side * min(max_abs_exposure, units * unit_exp)
        else:
            exposure = 0.0

        rows.append({
            "time":ts, "turtle_side":side, "turtle_system":system,
            "turtle_units":units, "turtle_N":n, "turtle_exposure_signal":exposure,
        })

    daily_sig = pd.DataFrame(rows).set_index("time")
    h4 = _ohlc_resample(df1h, "4h", "left")
    left = h4[["open","high","low","close"]].reset_index().rename(columns={h4.index.name or "index":"time"})
    right = daily_sig.reset_index().sort_values("time")
    x = pd.merge_asof(left.sort_values("time"), right, on="time", direction="backward").set_index("time")
    # One complete 4H execution delay after the daily signal timestamp.
    for c in ["turtle_side","turtle_system","turtle_units","turtle_N","turtle_exposure_signal"]:
        x[c] = x[c].shift(1)
    return x.fillna({"turtle_side":0,"turtle_system":0,"turtle_units":0,"turtle_exposure_signal":0.0})


def evaluate_exposure_4h(h4: pd.DataFrame, exposure: pd.Series, costs: CostModel,
                         initial: float, name: str, extra_delay: int = 0) -> tuple[pd.DataFrame, dict]:
    """Evaluate an already-causal exposure stream from 4H open to next 4H open."""
    idx = h4.index.intersection(exposure.index)
    px = h4.loc[idx, "open"].astype(float)
    exp = exposure.reindex(idx).fillna(0.0).astype(float)
    if extra_delay > 0:
        exp = exp.shift(extra_delay).fillna(0.0)
    r = px.pct_change().fillna(0.0)
    delta = exp.diff().abs().fillna(exp.abs())
    cost_rate = (costs.fee_bps + costs.slippage_bps) / 10000.0
    net = exp.shift(1).fillna(0.0) * r - delta * cost_rate
    equity = initial * (1.0 + net).cumprod()
    dd = equity / equity.cummax() - 1.0
    days = max((idx[-1]-idx[0]).total_seconds()/86400.0, 1.0)
    years = days/365.2425
    cagr = (equity.iloc[-1]/initial)**(1/years)-1 if equity.iloc[-1] > 0 else -1.0
    daily = equity.resample("1D").last().dropna().pct_change().dropna()
    sharpe = float(daily.mean()/daily.std()*np.sqrt(365)) if len(daily)>20 and daily.std()>0 else np.nan
    gp = float(daily[daily>0].sum()) if len(daily) else 0.0
    gl = float(-daily[daily<0].sum()) if len(daily) else 0.0
    ...
    m = {
        ...
        "max_drawdown":float(dd.min()),
        "sharpe_365":sharpe,
        "profit_factor_daily":float(pf) if np.isfinite(pf) else np.nan,
        "avg_abs_exposure":float(exp.abs().mean()),
        "max_abs_exposure":float(exp.abs().max()),
        "turnover_x_initial":float(delta.sum()),
    }
    eq = pd.DataFrame({"equity":equity,"exposure":exp,"ret":net,"drawdown":dd})
    return eq, m


def run_integrated_v2(data: pd.DataFrame, v92_eq: pd.DataFrame, costs: CostModel,
                      initial: float, out: Path):
    """Run the explicit V9.2 / Turtle / Book integration research matrix."""
    turtle = build_original_turtle_4h(data, risk_per_unit=0.0075, max_units=4, max_abs_exposure=1.50)
    book = build_book_hierarchy_4h(data)
    h4 = _ohlc_resample(data, "4h", "left")
    idx = h4.index.intersection(v92_eq.index).intersection(turtle.index).intersection(book.index)
    h4 = h4.loc[idx]
    v92 = v92_eq.target_exposure.reindex(idx).fillna(0.0).clip(-1.5,1.5)
    texp = turtle.turtle_exposure_signal.reindex(idx).fillna(0.0).clip(-1.5,1.5)
    bside = book.book_side_live.reindex(idx).fillna(0.0)
    bconf = book.book_conf_live.reindex(idx).fillna(0.0)

    # A: exact V9.2 live exposure stream as produced by its own engine.
    variants = {"V92_BASELINE_STREAM": v92}

    # B: original Turtle BTC adaptation, independent.
    variants["TURTLE_ORIGINAL_BTC"] = texp

    # C: V9.2 + Turtle breakout direction. Long confirmation increases conviction;
    #    short Turtle signals de-risk V9.2 rather than instantly flipping short.
    c = v92.copy()
    c = np.where(texp > 0, np.maximum(c, np.minimum(1.0, texp*0.70)), c)
    c = np.where(texp < 0, c*0.20, c)
    variants["V92_TURTLE_BREAKOUT"] = pd.Series(c,index=idx).clip(-1.25,1.25)

    # D: V9.2 + Turtle ATR sizing/pyramiding. Winner-funded Turtle size is added
    #    only when Turtle agrees with V9.2 long regime; disagreement de-risks.
    d = v92.copy()
    add = np.maximum(texp, 0.0) * 0.55
    d = np.where(texp > 0, np.minimum(1.35, d + add), np.where(texp < 0, d*0.15, d))
    variants["V92_TURTLE_PYRAMID"] = pd.Series(d,index=idx).clip(-1.35,1.35)

    # E: BOOK + TURTLE hybrid. Book major side is the directional gate.
    e = pd.Series(0.0,index=idx)
    long_mask = (bside > 0) & (texp > 0)
    short_mask = (bside < 0) & (texp < 0)
    e.loc[long_mask] = np.minimum(1.25, 0.35 + 0.75*np.abs(texp.loc[long_mask])*(0.6+0.4*bconf.loc[long_mask]))
    e.loc[short_mask] = -np.minimum(1.00, 0.30 + 0.60*np.abs(texp.loc[short_mask])*(0.6+0.4*bconf.loc[short_mask]))
    variants["BOOK_TURTLE_HYBRID"] = e

    # F: HIGH-CAGR track. Blend V9.2 core with full Turtle winner pyramids;
    #    allow shorts only when BOTH Turtle and book major regime are short.
    f = v92.copy()*0.75
    both_long = (texp > 0) & (bside >= 0)
    both_short = (texp < 0) & (bside < 0)
    f = pd.Series(f,index=idx)
    f.loc[both_long] = np.minimum(1.75, f.loc[both_long] + 0.95*np.abs(texp.loc[both_long]))
    f.loc[both_short] = -np.minimum(1.35, 0.90*np.abs(texp.loc[both_short]) + 0.20*bconf.loc[both_short])
    # disagreement -> retain only 20% of V9.2 risk
    disagree = ((texp < 0) & (bside >= 0)) | ((texp > 0) & (bside < 0))
    f.loc[disagree] = 0.20*v92.loc[disagree]
    variants["HIGH_CAGR_HYBRID"] = f.clip(-1.75,1.75)

    rows=[]
    for name, exp in variants.items():
        eq,m = evaluate_exposure_4h(h4, exp, costs, initial, name)
        eq.to_csv(out/f"integrated_equity_{name.lower()}.csv")
        stress_eq,stress_m = evaluate_exposure_4h(
            h4, exp, CostModel(costs.fee_bps*2,costs.slippage_bps*2), initial, name+"_2X_COST"
        )
        delay_eq,delay_m = evaluate_exposure_4h(h4, exp, costs, initial, name+"_PLUS4H", extra_delay=1)
        # Frozen recent-OOS slice: final 25% of available 4H history.
        cut = max(int(len(h4)*0.75),1)
        oidx = h4.index[cut:]
        oeq,om = evaluate_exposure_4h(h4.loc[oidx], exp.reindex(oidx), costs, initial, name+"_OOS25")
        row = {**m,
               "cost2x_cagr":stress_m["cagr"],"cost2x_mdd":stress_m["max_drawdown"],
               "delay4h_cagr":delay_m["cagr"],"delay4h_mdd":delay_m["max_drawdown"],
               "oos25_cagr":om["cagr"],"oos25_mdd":om["max_drawdown"],
               "gate_mdd15":abs(m["max_drawdown"])<=0.15,
               "gate_mdd20":abs(m["max_drawdown"])<=0.20,
               "gate_cagr20_mdd15":m["cagr"]>=0.20 and abs(m["max_drawdown"])<=0.15,
               "gate_cagr30_mdd20":m["cagr"]>=0.30 and abs(m["max_drawdown"])<=0.20}
        rows.append(row)
    comp = pd.DataFrame(rows).sort_values(["gate_mdd20","cagr"],ascending=[False,False])
    comp.to_csv(out/"INTEGRATED_V2_COMPARISON.csv",index=False)
    turtle.to_csv(out/"TURTLE_ORIGINAL_SIGNAL_4H.csv")
    book.to_csv(out/"BOOK_HIERARCHY_SIGNAL_4H.csv")
    return comp
...
def main():
    ...
    integrated_v2 = run_integrated_v2(data, seq, costs, args.initial, out)
    print("\n=== INTEGRATED V2 HIGH-CAGR MATRIX ===")
    print(integrated_v2[["strategy","cagr","max_drawdown","sharpe_365","profit_factor_daily","cost2x_cagr","delay4h_cagr","oos25_cagr","gate_cagr20_mdd15","gate_cagr30_mdd20"]].to_string(index=False))
